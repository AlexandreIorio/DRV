...
int main(int argc, char *argv[]) {
    ...
    while (running) {
        ...
        long delay = (long_press(0, LONG_PRESS_DURATION) == 0 || long_press(1, LONG_PRESS_DURATION) == 0) ?
                     LONG_PRESS_DURATION : DELAY_US;
        usleep(delay);
    }
...
int initialize() {
    ...
    counter_ctl.base_addr = (uint8_t *)mmap(NULL, MAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, counter_ctl.uio_fd, 0);
    ...
    volatile uint32_t *btn_reg = (volatile uint32_t *)(counter_ctl.base_addr + BUTTON_OFFSET);
    volatile uint32_t *hex_reg_0_3 = (volatile uint32_t *)(counter_ctl.base_addr + HEX_OFFSET_0_3);
    volatile uint32_t *hex_reg_4_5 = (volatile uint32_t *)(counter_ctl.base_addr + HEX_OFFSET_4_5);
    volatile uint32_t *led_reg = (volatile uint32_t *)(counter_ctl.base_addr + LED_OFFSET);
