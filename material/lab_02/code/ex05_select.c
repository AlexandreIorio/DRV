...
int main(int argc, char *argv[]) {
  ...
  printf(
      "Press KEY1 to substract the value of the switches to the accumulator\n");
  printf(
  ...
  while (running) {
    ...
    if (ret > 0 && FD_ISSET(counter_ctl.uio_fd, &fds)) {
      ...
      if (nb == (ssize_t)sizeof(info)) {
        ...
        logMessage(ERROR, "Cannot read from UIO device\n");
        close(counter_ctl.uio_fd);
        exit(EXIT_FAILURE);
      }
    } else {
...
void divi(int value) {
  ...
  if (value == 0) {
    logMessage(WARNING, "Error: division by zero can be dangerous\n");
    return;
  }
  counter_ctl.value /= value;
}
